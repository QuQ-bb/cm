...
<div class="goodsQnaList">
	<table border="1" id="goodsQnaTable" width="800">
		...
		<c:forEach var="mygoodsQnaList" items="${mygoodsQnaList}">
		
			<tr <c:if test="${mygoodsQnaList.gdsqna_check == 0 }"> class="notcheck"</c:if>>
				<td style="text-align: center;">
					<!-- 번호 출력 부분 -->
					<input type="hidden" id="gdsqna_num" name="gdsqna_num" value="${mygoodsQnaList.gdsqna_num}">
					<c:out value="${num}"/>			
					<c:set var="num" value="${num-1}"/>	 
				</td>
				<td class="goodsQnaListName" title="${mygoodsQnaList.gds_name}"><nobr>${mygoodsQnaList.gds_name}</nobr></td>
				<td>${mygoodsQnaList.gdsqna_title}</td>
				<td>
					<fmt:formatDate value="${mygoodsQnaList.gdsqna_date}" pattern="yyyy-MM-dd HH:mm:ss"/> 
				</td>
				<td style="text-align: center;">
					<c:choose>
						<c:when test="${mygoodsQnaList.gdsqna_answer == 0}">
							미답변
						</c:when>
						<c:when test="${mygoodsQnaList.gdsqna_answer == 1}">
							답변완료
						</c:when>
					</c:choose>
				</td>
			...
			<tr>
				<td colspan="6">
					<div id="questiondetail${mygoodsQnaList.gdsqna_num}" class="hide"></div>
				</td>
			</tr>
		</c:forEach>
	</table>
